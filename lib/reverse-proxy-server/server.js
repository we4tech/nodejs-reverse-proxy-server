...
this.ReverseProxyServer = function(options) {
  ...
  this.serveProxyReverse = function(request, response) {
    ...
    if (backendServerConf) {
      if (serverUtil.isResourceUrl(backendServerConf.rootDir, request.url)) {
      ...
      this.mEvent.emit(EVENT_TYPE_CONFIGURATION_ERROR, response, 'No such virtual host `' + request.headers.host + '` was configured.');
    }
  };
