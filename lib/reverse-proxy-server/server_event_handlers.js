var ServerEventHandlers = function(server) {
    ...
    server.mEvent.on(EVENT_TYPE_PROCESS, function(request, response, buffer) {
        server.onProcessing(request, response, buffer);
    });

    server.onProcessing = function(request, response, buffer) {
        var headers = request.headers;
        server.handleWithinVirtualHostScope(request, response, headers.host, buffer);
    };

    server.handleWithinVirtualHostScope = function(request, response, host, buffer) {
        for (var vHostRegex in server.optVirtualHosts) {
            server.debug('Matching current host (' + host + ') with the regex(' + vHostRegex + ')');

            var regex = new RegExp(vHostRegex, 'i');

            if (host.match(regex)) {
                var backendServers = server.optVirtualHosts[vHostRegex];
                server.debug('Found Virtual Host, Listed Backend - ');
                console.log(backendServers);

                // Lookup backend server
                var backendServerConf = server.optBackendSelectionStrategy(server.optNodeCache, host, backendServers);
